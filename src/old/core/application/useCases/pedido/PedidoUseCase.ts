import { IPedidoRepository } from "@/old/core/ports/IPedidoRepository";
...
class PedidoUseCase implements IPedidoUseCase {
    ...
    constructor(
        produtosDoPedidoRepository: IProdutosDoPedidoRepository,
    ...
    async executeUpdatePedidoFinalizado(idPedido: number) {
        try {
            const response = await this.pedidoRepository.updatePedido(
                idPedido,
                "Finalizado"
            );
    ...
    async executeGetPedidoById(idPedido: number) {
        try {
            const response = await this.pedidoRepository.getPedidoById(
                idPedido
            );
    ...
    async executeGetPedidoByStatus(status: string) {
        try {
            const response = await this.pedidoRepository.getPedidosByStatus(
                status
            );
    ...
    async executeGetPedidoFakeCheckout(status: string) {
        try {
            const response =
                await this.pedidoRepository.getPedidoByStatusFakeCheckout(
                    status
                );
    ...
    executeAddProdutoAoPedido(idPedido: number, produtos: Produto[]) {
        try {
            const response = this.produtosDoPedidoRepository.create(
                idPedido,
                produtos
            );
